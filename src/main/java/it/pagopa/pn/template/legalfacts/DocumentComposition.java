...
public class DocumentComposition {
  ...
  // TODO aggiungere altri template, e vedere se passarli nella chiamata
  public enum TemplateType {
    REQUEST_ACCEPTED("documents_composition_templates/NotificationReceivedLegalFact.html"),
    DIGITAL_NOTIFICATION_WORKFLOW(
        "documents_composition_templates/PecDeliveryWorkflowLegalFact.html"),
    ANALOG_NOTIFICATION_WORKFLOW_FAILURE(
        "documents_composition_templates/AnalogDeliveryWorkflowFailureLegalFact.html"),
  ...
  // TODO vedere se servirà la mappa o altro
  ...
  private static BaseUriAndTemplateBody preloadTemplate(TemplateType templateType)
      throws IOException {
    ...
    log.debug("Template with templateResourceName={} located at URL={}", templateResourceName,
        templateUrl);
  ...
  public byte[] executePdfTemplate(String templateName, Map<String, Object> mapTemplateModel) throws IOException {
    String html = executeTextTemplate(templateName, mapTemplateModel);
    String baseUri = baseUris.get(templateName);
  ...
  public int getNumberOfPageFromPdfBytes(byte[] pdf) {
    try (PDDocument document = PDDocument.load(pdf)) {
      ...
      throw new PnInternalException("Cannot get numberOfPages for pdf " + this.getClass(),
          ex.getMessage());
    }
  }
